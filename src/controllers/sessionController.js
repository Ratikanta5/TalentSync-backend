const { chatClient, streamClient } = require("../config/stream/stream");
...
module.exports.removeParticipant = async (req, res) => {
  try {
    ...
    if (session.participant) {
      const participantClerkId = session.participant;
      session.participant = null;
      await session.save();

      // Remove from chat channel
      const channel = chatClient.channel("messaging", session.callId);
      await channel.removeMembers([participantClerkId.toString()]);
    }

    res.status(200).json({ session, message: "Participant removed successfully" });
  } catch (error) {
